...
goal_model_dir = os.path.join(os.path.split(os.path.realpath(__file__))[0], '..', 'turtlebot3_simulations','turtlebot3_gazebo', 'models', 'goal_target', 'model.sdf')
...
class Env(Node):
    ...
    def step(self, action, past_action):
        ...
        if data:
            ...
            if elapsed_time >= EPISODE_TIMEOUT:  # If timeout reached
    ...
    def setReward(self, done, arrive):
        # Remove rclpy.spin_once(self) from here
        ...
        if arrive:
            ...
            try:
                ...
                print("Building the target2")
                target = SpawnEntity.Request()
                target.name = "goal_target"
                target.xml = goal_urdf
                self.goal_position.position.x = np.random.uniform(-2.4, 2.4)
                self.goal_position.position.y = np.random.uniform(-2.4, 2.4)
                target.initial_pose = self.goal_position
                self.goal.call_async(target)
            except Exception as e:
                self.get_logger().error(f"Error reading goal model: {str(e)}")
    ...
    def reset(self):
        ...
        # print("Resetting2.5")
        try:
            # print("Building the target1")
            goal_urdf = open(goal_model_dir, "r").read()
            target = SpawnEntity.Request()
            target.name = 'goal_target'
            target.xml = goal_urdf
            obstacle_regions = [
                (-0.25, 0.25, -0.25, 0.25),         # Obstacle at (0, 0)
                (1.0, 1.5, 1.0, 1.5),               # Obstacle at (1.25, 1.25)
                (1.0, 1.5, -1.5, -1.0),             # Obstacle at (1.25, -1.25)
                (-1.5, -1.0, 1.0, 1.5),             # Obstacle at (-1.25, 1.25)
                (-1.5, -1.0, -1.5, -1.0),           # Obstacle at (-1.25, -1.25)
            ]
    ...
    def reset2(self):
        ...
        try:
            ...
            while True:
                x = np.random.uniform(1.8, 2.4)
                y = np.random.uniform(-2.0, 2.4)
                # x = 1.8
                # y = 0.0
